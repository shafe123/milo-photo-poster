...
def upload_to_blob_storage(images: List[tuple], connection_string: str, container_name: str, prefix: str = "negative-examples"):
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Download cat images for negative training examples")
    ...
    parser.add_argument("--output-dir", default="negative_cat_images", help="Local output directory")
    parser.add_argument("--upload", action="store_true", help="Upload to Azure Blob Storage")
    parser.add_argument("--connection-string", help="Azure Storage connection string (from env if not provided)")
    ...
    if args.upload:
        ...
        if not connection_string:
            ...
            print("  Set AZURE_STORAGE_CONNECTION_STRING environment variable or use --connection-string")
        else:
    ...
    print(f"  2. Go to https://www.customvision.ai/")
    print(f"  3. Open your project: milo-description")
    print(f"  4. Upload images and tag them as 'Negative' (or create a new tag like 'other-cats')")
    print(f"  5. Train a new iteration")
    print("=" * 80)
