...
class TestFluxGeneration:
    ...
    def test_generate_ai_image_includes_reference_image(
        self, function_app_module, mock_blob_client
    ):
        """Test that reference image is included when blob storage is available"""
        with patch("function_app.requests.post") as mock_post:
            mock_response = Mock()
            mock_response.status_code = 200
            mock_response.json.return_value = {"data": [{"b64_json": "ZmFrZV9pbWFnZV9ieXRlcw=="}]}
            mock_post.return_value = mock_response
    ...
    def test_generate_ai_image_includes_caption_context_in_prompt(self):
        ...
        with (
            patch("function_app.FLUX_API_URL", "https://example.com/flux"),
    ...
    def test_generate_ai_image_handles_api_error(self, function_app_module):
        ...
        with patch("function_app.requests.post") as mock_post:
            ...
            result = function_app_module.generate_ai_image(client=Mock(), image_model="FLUX.2-pro")
