...
def test_milo_detection_real_api_call():
    ...
    is_milo_present, confidence = check_milo_in_photo(mock_blob_client, TEST_IMAGE_URL)
...
def test_milo_detection_with_blob_storage_image():
    ...
    function_app.CUSTOM_VISION_PREDICTION_KEY = key
    function_app.CUSTOM_VISION_PROJECT_ID = project_id
    function_app.CUSTOM_VISION_ITERATION_NAME = os.environ.get(
    ...
    # Make the actual API call
    is_milo_present, confidence = check_milo_in_photo(blob_client, image_url)
    ...
    assert cached_iteration == expected_iteration, (
        f"Expected iteration {expected_iteration}, got {cached_iteration}"
    )
